...
const FeeReportTable = () => {
  ...
  const bookCondition = useMemo(
    () =>
      bookConditionList.map((cond) => ({
        ...cond,
        fullName: `${cond.firstName} ${cond.middleName} ${cond.lastName}`,
      })),
    [bookConditionList]
  );
  ...
  const customColumns = useMemo<MRT_ColumnDef<ICirculation>[]>(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          const otherBadge = row.getValue("bookCondition");

          switch (otherBadge) {
            case (otherBadge as string).toLowerCase().includes("damage"): {
              return (
                <Badge
                  color="#C31209"
                  tt={"inherit"}
            ...
            case (otherBadge as string).toLowerCase().includes("return"): {
              return (
                <Badge
                  color="#027127"
                  tt={"inherit"}
            ...
            case (otherBadge as string).toLowerCase().includes("overdue"): {
              return (
                <Badge
                  color="#E39500"
                  tt={"inherit"}
  ...
  const exportToPDF = async (data: Row<ICirculation>[]) => {
    ...
    (doc as jsPDF & { autoTable: autoTable }).autoTable({
      ...
      willDrawPage: () => {
        ...
        doc.setFontSize(16);
        doc.text(
          `Binangonan Catholic College Collection Fee Report `,
          docWidth / 2,
          currentHeight - 5,
          {
            align: "center",
