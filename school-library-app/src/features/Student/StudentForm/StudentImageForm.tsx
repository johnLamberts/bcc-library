...
const UserImageForm = <TData extends MRT_RowData>({
  ...
  const isNewEntity = !isEditing || !row?.original.studentImage;
  const hasNewValue = isEditing && row?.original.studentImage !== imgSrc;
  ...
  const displayCameraImage = useMemo(() => {
    ...
    if (hasNewValue) {
      // const updatedValue = getValues("studentImage") || imgSrc;
  ...
  useEffect(() => {
    if (isNewEntity || hasNewValue) {
      ...
      if (imgSrc instanceof File && imgSrc !== null) {
        setValue("studentImage", updatedValue);
      } else if (
        typeof imgSrc === "string" &&
        imgSrc.startsWith("data:image")
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        setValue("studentImage", imgSrc as any);
      }
    } else {
      console.log("its nothing here");
      // If it's not a new entity and there's no new value, set the previous value
      setValue("studentImage", row?.original.studentImage || null);
    }
  }, [
    setValue,
    imgSrc,
  ...
  return (
    <>
      <Form.Box mt={"md"}>
        <Form.Title>Student Image</Form.Title>
        <Form.Grid p={"lg"}>
          <Form.Col span={{ base: 12 }}>
            ...
            >
              ...
              {row?.original.studentImage === getValues("studentImage") &&
              isEditing ? (
                <Image src={getValues("studentImage")} w={250} h={150} />
              ) : null}
            </Box>
