...
interface teacherImageProps<TData extends MRT_RowData> {
  table?: MRT_TableInstance<TData>;
  row?: MRT_Row<TData>;
}

const TeacherImage = <TData extends MRT_RowData>({
  table,
  row,
}: teacherImageProps<TData>) => {
  const { control, setValue, getValues } = useFormContext();
  const [imgSrc, setImgSrc] =
    useState<Partial<null | undefined | string | File>>(null);
  const isEditing = table?.getState().editingRow?.id === row?.id;
  const isNewEntity = !isEditing || !row?.original.teacherImage;
  const hasNewValue = isEditing && row?.original.teacherImage !== imgSrc;
  ...
  const displayCameraImage = useMemo(() => {
    if (isNewEntity) {
      ...
      if (imgSrc instanceof File) {
        return (
          <Image
            src={URL.createObjectURL(getValues("teacherImage") || imgSrc)}
            alt="webcam"
          />
        );
      }
    ...
    if (hasNewValue) {
      // const updatedValue = getValues("teacherImage") || imgSrc;
  ...
  useEffect(() => {
    if (isNewEntity || hasNewValue) {
      ...
      if (imgSrc instanceof File && imgSrc !== null) {
        setValue("teacherImage", updatedValue);
      } else if (
        typeof imgSrc === "string" &&
        imgSrc.startsWith("data:image")
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        setValue("teacherImage", imgSrc as any);
      }
    } else {
      console.log("its nothing here");
      // If it's not a new entity and there's no new value, set the previous value
      setValue("teacherImage", row?.original.teacherImage || null);
    }
  }, [
    setValue,
    imgSrc,
  ...
  return (
    <>
      <Form.Box mt={"md"}>
        <Form.Title>Teacher Image</Form.Title>
        <Form.Grid p={"lg"}>
          <Form.Col span={{ base: 12 }}>
            ...
            >
              ...
              {row?.original.teacherImage === getValues("teacherImage") &&
              isEditing ? (
                <Image src={getValues("teacherImage")} w={250} h={150} />
              ) : null}
            </Box>
